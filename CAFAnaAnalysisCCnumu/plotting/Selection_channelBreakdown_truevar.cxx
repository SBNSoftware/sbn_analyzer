...
int main(int args, char* argv[]){
 ...
 for(int iVar=0; iVar<vars.size(); iVar++){//(string var: vars){
    ...
    for(int jChan=channels.size()-1; jChan>=0; jChan--){//so SIS is on top of legend
      ...
      string name=  var+"__trueVars_truthIs_"+chan;
      cout<<name<<endl;
  ...
  for( string var: vars){
    ...
    */
    string plotname="selection_chanStack_true"+var;
    plotter.DrawStackedMC(hchannelArrays[var], 1. , "TL");//, 152, 1);
    ...
    plotter.MultiPrint(c, plotname+"__uncert");
