...
void Selection_notruth() {
    ...
    static const std::vector<std::string> VarNames=GetSISVarNames();
    ...
    for (std::size_t i = 0; i < Vars.size(); i++) {
      ...
      auto RecoTrueSignals =std::make_unique<Spectrum>(VarLabels.at(i), VarBins.at(i), NuLoader, std::get<0>(Vars.at(i)), kNoSpillCut, kRecoIsSignal);  //std::make_unique<Spectrum> (VarLabels.at(i), VarBins.at(i), NuLoader, std::get<0>(Vars.at(i)), kNoSpillCut, kRecoIsTrueReco); 
      auto RecoBkgSignals = std::make_unique<Spectrum>(VarLabels.at(i), VarBins.at(i), NuLoader, std::get<0>(Vars.at(i)), kNoSpillCut, kRecoIsSignal); //std::make_unique<Spectrum>(VarLabels.at(i), VarBins.at(i), NuLoader, std::get<0>(Vars.at(i)), kNoSpillCut, kRecoIsBackground); 
    ...
    for (std::size_t i = 0; i < Vars.size(); i++) {
      ...
      // Save as pdf
      PlotCanvas->SaveAs(plotdir+Form("/Figs/CAFAna/Selection%s.pdf", var.c_str() ) );
